...
- `--pr <N>` → check that PR out first (into a clean tree only; a dirty tree is a stop, not a stash).
...
| Posts anything | **no** | thread replies + resolutions | thread replies + resolutions |
