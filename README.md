...
| [`deliver`](./skills/deliver/SKILL.md) | Front-load CI locally, open/update a PR, request a reviewer, address feedback over at most 3 review rounds (5 while blocking findings keep arriving), auto-merge if changes since invocation are minimal (or stop at ready-for-review with `--no-merge`). `--base <branch>` targets a parent branch instead of the repo default, so a stacked PR shows only its own increment. |
