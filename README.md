...
| [`deliver`](./skills/deliver/SKILL.md) | Front-load CI and one fresh-context review locally, open/update a PR, request a reviewer, address feedback (verified first, threads resolved before any re-request) over at most 3 review rounds (5 while blocking findings keep arriving), auto-merge if changes since invocation are minimal (or stop at ready-for-review with `--no-merge`). `--base <branch>` targets a parent branch instead of the repo default, so a stacked PR shows only its own increment. |
...
| [`om-review-loop`](./skills/om-review-loop/SKILL.md) | Harden an Open Mercato branch before it goes for review: run `om-code-review` in **fresh subagent contexts** each round — no fix rationale, no round number, nothing that tells a reviewer it is meant to find nothing — dedupe against every finding ever raised (rejected ones included, or they return forever), refute-by-default before spending a code change, fix, repeat until N *consecutive* rounds turn up nothing new and the full validation gate passes. Reports the findings-per-round curve, hands judgement calls back instead of deciding them, and says what N quiet rounds do and don't prove. Local only — posts nothing, pushes nothing, merges nothing. |
