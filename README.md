...
| [`kickoff`](./skills/kickoff/SKILL.md) | Idea / brainstorm brief / ticket → ready-for-review PR: decides plan depth itself (spec in the repo's spec location, or straight to code), implements with tests, then runs `deliver --no-merge` for checks, the local review loop, the PR, and CI. Never merges. |
...
| [`pickup`](./skills/pickup/SKILL.md) | One ticket → a ready-for-review PR, or an honest *don't*: `ticket-refresh` first, then verify the premise **measured, not read** (is the problem still there, is the named cause the cause, what does doing nothing cost), survive a fresh-context challenger, reach one verdict (do / re-scope / abandon / already-done), write a plan brief with its open questions, batch them into one interactive round, get an explicit go — then `kickoff` → `deliver --no-merge`. `--unattended` records assumptions instead of asking and stops on anything that forks the product. Never merges. |
| [`deliver`](./skills/deliver/SKILL.md) | Front-load CI locally, get the change **reviewed and fixed before the first push** via [`review-loop`](./skills/review-loop/SKILL.md), open/update a PR whose body records what that review found and what it handed back, work the CI loop, auto-merge if changes since invocation are minimal (or stop at ready-for-review with `--no-merge`). A bot review on the PR is opt-in (`--bot-review`, or a `reviewer` profile knob), not the default path — a reviewer behind a ten-minute round-trip is evidence, not a first pass. `--base <branch>` targets a parent branch instead of the repo default, so a stacked PR shows only its own increment; `--review-rounds N` (0–10, default 3) caps the local loop, and `0` skips it and stops at ready-for-review. |
...
| [`ticket-polish`](./skills/ticket-polish/SKILL.md) | `ticket-refresh`'s complement: refresh makes a body true, polish makes it legible. Picks the body shape the ticket's situation calls for — problem-first when nothing else records the problem, hub when a live spec or parent ticket carries it — then restructures into the house section grammar (`🎯` `🔧` `🔍` `⚠️` `✅`, emoji and order fixed, heading words written for this ticket, empty sections dropped): one canonical enumeration instead of parallel numbering, open work leads, history compressed to its surviving reasoning, a checkable DoD of only the remaining work, title re-trued. Two invariants it will add rather than only subtract: a problem statement and a DoD. Form only, facts unchanged — it runs `ticket-refresh` itself when the body's facts have gone stale, so a neglected ticket needs one invocation, not two. |
...
| [`docs-audit`](./skills/docs-audit/SKILL.md) | Audit a repo's documentation and agent instructions against the house conventions, then fix the mechanical and propose the rest: instruction-budget overflow (the rules an agent never receives), `AGENTS.md`/`CLAUDE.md` drift, commands that no longer resolve, dead links, unindexed docs, spec-convention breaks, state docs narrating their own history. Ships a CI gate so the rules hold without re-running it. `--audit-only` writes nothing. |
| [`blacksmith`](./skills/blacksmith/SKILL.md) | Move a repo's GitHub Actions compute to Blacksmith runners: inventory every job, migrate what builds/runs/tests the app at the same vCPU (image builds get the sticky-disk Docker actions, external layer cache dropped), leave glue, deploy, self-hosted and credential-bound jobs on GitHub, capture a timing baseline, and hand off to `deliver --no-merge` with the decision table in the PR. `--audit-only` writes nothing. |
...
| [`review-loop`](./skills/review-loop/SKILL.md) | Get a change reviewed and fixed until it comes back clean, from a swappable source: **fresh subagent contexts** each round (the default — no fix rationale, no round number, nothing that tells a reviewer it is meant to find nothing), a PR review bot, or a human. One loop around all three: dedupe against every finding ever raised (the refuted included, or they return forever), refute-by-default before spending a code change, fix, repeat until N *consecutive* rounds turn up nothing new and the gate passes. Rubric is `om-code-review` where installed, else the built-in `code-review`. Reports the findings-per-round curve, hands judgement calls back instead of deciding them, writes a `state.json` a caller can merge on, and says what N quiet rounds do and don't prove. The local source publishes nothing at all; the PR-attached ones push only what the reviewer must re-read. Never merges. |
...
| [`video-brief`](./skills/video-brief/SKILL.md) | A talk, webinar, panel or call recording → a brief that replaces watching it. Transcribes locally with Whisper when the source has no real subtitles (a 98-minute webinar in 1–4 minutes on Apple Silicon, audio never leaves the machine), un-rolls scrolling auto-captions into prose, then writes notes organized by the *argument* rather than the clock — with a timestamp on every claim, the sales pitch pulled into its own section, and every machine-heard proper noun marked unverified. Needs `yt-dlp` + `ffmpeg`. |
...
| [`grill-me`](./skills/grill-me/SKILL.md) | Interrogate a plan you already have, in rounds: map it as a design tree, ask every question whose prerequisites are settled at once (numbered, each with a recommended answer), look facts up via subagents instead of asking, stop only when no branch is left assumed. `brainstorm` asks *whether*; this asks *how*. Adapted from [mattpocock/skills](https://github.com/mattpocock/skills) (MIT). |
| [`handoff`](./skills/handoff/SKILL.md) | Compact the conversation into a handoff doc for a fresh session — goal, state, decisions and dead ends, open questions, suggested skills — referencing existing artifacts instead of copying them, secrets redacted. Writes to `.context/handoffs/` (else the OS temp dir). Manual-invoke only. Adapted from [mattpocock/skills](https://github.com/mattpocock/skills) (MIT). |
...
`explain`, `brainstorm`, `kickoff`, `overnight`, `pickup`, `deliver`, `upstream-pr`, `pr-polish`, `ticket-refresh`, `ticket-polish`, `spec-polish`, `docs-audit`, `blacksmith`, `review-queue`, `review-loop`, `bug-hunt`, `flake-hunt`, `project-status`, `design-polish`, `design-explore`, `prior-art`, and `radar` are **repo-agnostic** — they derive
...
The repo-agnostic skills (`deliver`, `review-loop`, `upstream-pr`, `bug-hunt`, `flake-hunt`;
`explain` to a lesser extent) derive most specifics at runtime from the consuming repo's `AGENTS.md` /
`CLAUDE.md` / `package.json` scripts. For knobs that aren't derivable from docs, add a
**`## Skill profile`** section to the consuming repo's root `AGENTS.md`. Recognized knobs:
...
- **radar** — where `radar` files its notes on artifacts that arrive from outside (a posts
  directory + its index `README.md`). Default `docs/radar/` when the repo has a `docs/`, else
  `.context/radar/`, else the skill asks. Worth setting alongside a line about **who the
  decisions are for** — the shape the skill weighs external advice against (team size, how the
  work gets done, how money is earned); it reads the repo's agent and company docs for this, and
  a context with no such docs has to be told.
- **PR review bot** login — `reviewer`, e.g. `copilot-pull-request-reviewer`. Set it only if
  ...
  an alias like `@copilot`, which requests fine and then matches no review.
- **Human reviewers** — `reviewers`, requested only when branch protection blocks the merge on
  a missing approval. Unset, a candidate is derived from who reviews the repo's merged PRs.
