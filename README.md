...
| [`docs-audit`](./skills/docs-audit/SKILL.md) | Audit a repo against the documentation contract *it already wrote for itself*, then repair it: the root→working-dir agent-doc chains every session loads, spec numbering and template conformance (conventions **derived** per repo — three are in use across our four), docs nothing links to, and links that no longer resolve. Re-trues every rule against the code before touching it; reference content *moves* rather than being deleted. Offers a CI ratchet that freezes existing debt. Proposes; applies only on approval. |
