...
| [`agent-docs`](./skills/agent-docs/SKILL.md) | Audit and repair a repo's agent instruction files (`AGENTS.md`/`CLAUDE.md`): measure the root→working-dir chains every session actually loads, re-true each rule against the code before touching it, then classify every section (rule / router / reference / essay / stale) — reference content *moves* to a referenced doc, incident essays compress to their one rule. Offers a CI ratchet that freezes existing debt. Proposes; applies only on approval. |
