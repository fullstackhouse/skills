...
| [`competitor-teardown`](./skills/competitor-teardown/SKILL.md) | `prior-art`'s commercial counterpart, done hands-on: sign into the alternative the buyer is actually trialling, work *their* stated jobs through its UI (failure states and upgrade walls included), find the plan gate under the feature they need and price it at the headcount they said they're growing to, then quote the vendor's own scope disclaimer — the most credible sentence in the analysis. Ships the verdict and the screenshot evidence in the same pass, because a capture directory with no synthesis is a debt nobody repays. Concedes ground where they're genuinely better. Buys nothing, posts nothing. |

`explain`, `brainstorm`, `kickoff`, `overnight`, `deliver`, `upstream-pr`, `pr-polish`, `ticket-refresh`, `ticket-polish`, `spec-polish`, `docs-audit`, `review-queue`, `bug-hunt`, `flake-hunt`, `project-status`, `design-polish`, `design-explore`, `prior-art`, and `competitor-teardown` are **repo-agnostic** — they derive
