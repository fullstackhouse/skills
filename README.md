...
| [`ticket-polish`](./skills/ticket-polish/SKILL.md) | `ticket-refresh`'s complement: refresh makes a body true, polish makes it legible. Picks the body shape the ticket's situation calls for — problem-first when nothing else records the problem, hub when a live spec or parent ticket carries it — then restructures: one canonical enumeration instead of parallel numbering, open work leads, history compressed to its surviving reasoning, a checkable DoD of only the remaining work, title re-trued. Two invariants it will add rather than only subtract: a problem statement and a DoD. Form only, facts unchanged — it runs `ticket-refresh` itself when the body's facts have gone stale, so a neglected ticket needs one invocation, not two. |
